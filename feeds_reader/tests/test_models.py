...
class FeedTest(TestCase):
    ...
    def setUp(self):
        self.feed = Feed.objects.create(xml_url='http://localhost:{}/test/feed'.format(PORT))
    ...
    def test_feed_str(self):
        ...
        self.assertEqual(feed_str, 'Test Feed',
                         'Feed: Unexpected __str__ value: Got {} expected "Test Feed"'.format(feed_str))
...
class EntryTest(TestCase):
    ...
    def setUp(self):
        self.feed = Feed.objects.create(xml_url='http://localhost:{}/test/feed'.format(PORT))
        self.entry = Entry.objects.create(feed=self.feed, title='Test Entry', link='http://example.com/test')

    def test_entry_str(self):
        """Retrieve Entry object's str string."""
        entry_str = self.entry.__str__()
        self.assertEqual(entry_str, 'Test Entry',
                         'Entry: Unexpected __str__ value: Got {} expected "Test Entry"'.format(entry_str))
