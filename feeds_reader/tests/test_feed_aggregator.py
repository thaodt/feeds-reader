"""Feeds Reader Retrieval Unit Test."""
...
from ..feed_aggregator import retrieve_feed
...
class RetrieveFeedTest(TestCase):
    ...
    def test_published_time(self):
        ...
        with patch('feeds_reader.utils.feedparser.parse', self.parse_mock):
            # No published time in DB
            feed_mock = self.feed_mock
            feed_mock.published_time = None
            with patch('sys.stdout', new=StringIO()):
                retrieve_feed(feed_mock, verbose=True)
    ...
    def test_missing_attribute(self):
        ...
        with patch('feeds_reader.utils.feedparser.parse', parse_mock):
            with patch('sys.stdout', new=StringIO()):
                retrieve_feed(self.feed_mock, verbose=True)
...
@patch('feeds_reader.utils.feedparser.parse')
class RetrieveFeedBozoExceptionTest(TestCase):
    """
    Test retrieving feeds where Bozo Exception returned.
    """
...
class RetrieveEntriesTest(TestCase):
    def setUp(self):
        # Create feedparser.parse_mock object
        parse_mock = Mock()
        del parse_mock.return_value.feed.bozo_exception
    ...
    def test_feed_entry_blank_title(self):
        ...
        with patch('feeds_reader.utils.feedparser.parse', parse_mock):
            with patch('feeds_reader.utils.Entry', db_entry_mock):
                with patch('sys.stdout', new=StringIO()):
                    retrieve_feed(self.feed_mock, verbose=True)
