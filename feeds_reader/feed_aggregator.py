    ...
    if last_entry:
        ...
        if last_entry in entry_list:
            ...
            for i in range(last_entry_pos + 1):
                if entry_list[i].read_flag is False:
...
def retrieve_feed(xml_url, verbose=False):
    """
    ...
    if hasattr(parsed.feed, 'bozo_exception'):
        # Malformed feed
        msg = 'FeedsReader retrieve_feeds found Malformed feed, "%s": %s' % (xml_url, parsed.feed.bozo_exception)
        logger.warning(msg)
        # if verbose:
        #     print(msg)
        return
    ...
    for attr in ['title', 'title_detail', 'link']:
        if not hasattr(parsed.feed, attr):
            msg = 'FeedsReader retrieve_feed. Feed "%s" has no %s' % (xml_url, attr)
            logger.error(msg)
            # if verbose:
            #     print(msg)
            return
    db_feed, feed_created = Feed.objects.get_or_create(xml_url=xml_url)
    if 'text/plain' == parsed.feed.title_detail.type:
    ...
    for i, entry in enumerate(parsed.entries):
        ...
        for attr in ['title', 'title_detail', 'link', 'description']:
            if not hasattr(entry, attr):
                msg = 'FeedsReader retrieve_feed. Entry "%s" has no %s' % (entry.link, attr)
                logger.error(msg)
                if verbose:
                    print(msg)
                missing_attr = True
        if missing_attr:
            continue
        if entry.title == "":
            msg = 'FeedsReader retrieve_feed. Entry "%s" has a blank title' % (entry.link)
            if verbose:
                print(msg)
            logger.warning(msg)
            continue
        ...
        if created:
            if hasattr(entry, 'published_parsed'):
                if entry.published_parsed is None:
                    ...
                    try:
                        ...
                        now = now.replace(tzinfo=pytz.utc).astimezone(pytz_timezone)
                    except pytz.exceptions.AmbiguousTimeError:
                        published_time = pytz_timezone.localize(published_time, is_dst=False)
            ...
            # Lots of entries are missing description attributes. Escape their content by default
